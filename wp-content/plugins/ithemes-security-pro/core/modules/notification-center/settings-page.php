		...
		<div class="itsec-notification-center-mail-errors-container">
			<?php foreach ( $errors as $id => $error ):
				...
				if ( is_wp_error( $details ) ) {
					...
					$message = __( 'Unknown error encountered while sending.', 'it-l10n-ithemes-security-pro' );
				}
				?>
				<div class="notice notice-alt notice-error below-h2 itsec-is-dismissible itsec-notification-center-mail-error" data-id="<?php echo esc_attr( $id ); ?>">
					<p><?php printf( esc_html__( 'Error while sending %1$s notification at %2$s: %3$s', 'it-l10n-ithemes-security-pro' ), '<b>' . $strings['label'] . '</b>', '<b>' . ITSEC_Lib::date_format_i18n_and_local_timezone( $error['time'] ) . '</b>', $message ); ?></p>
				</div>
			<?php endforeach; ?>
		</div>
		<?php
