...
final class ITSEC_Grading_System_Active {
	...
	public function maybe_send_grade_report( $job ) {
		...
		if ( $data['at'] + DAY_IN_SECONDS < ITSEC_Core::get_current_time_gmt() ) {
		...
		$last_sent = get_site_option( 'itsec_grade_report_last_sent', 0 );

		$not_before = $last_sent + DAY_IN_SECONDS;
	...
	private function send_grade_report( $report, $previous ) {
		...
		ITSEC_Core::get_notification_center()->send( 'grade-report-change', $mail );

		update_site_option( 'itsec_grade_report_last_sent', ITSEC_Core::get_current_time_gmt() );
	}
